...
for subject in participant_list:
	...
	subject_id = participant_long[subject.find('sub-'), subject.find('sub-')+7]
	...
	zmap_data_binarised_inverted = nib.load('/project/3013068.03/RETROICOR/Example_Visualisation/{0}/'\
                                            'RETRO_vs_AROMA.feat/thresh_zfstat2.nii.gz'.format(participant)).get_fdata()
	...
	nib.save(zmap_nii, '/project/3013068.03/RETROICOR/Example_Visualisation/{0}/RETRO_vs_AROMA.feat'\
             '/binarised_thres_zfstat.nii.gz'.format(participant))
	
	# Load respective functional Data
	func_data = subject.get_func_data(session=ses_nr,run=2,task='RS', MNI=True)
	...
	melodic_mixing_matrix = pd.read_csv(glob.glob('/project/3013068.03/derivate/fmriprep/{0}/ses-mri0{1}/func/{0}_ses-mri0{1}_task'\
                                                  '-15isomb3TR2020TE28RS*run-2_echo-1_desc-MELODIC_mixing.tsv'.format(participant, ses_nr+1))[0],\
                                        header=None, sep='\t')
	...
	for x in range(0,np.shape(melodic_mixing_matrix)[1]):
		...
		sub_noise_components = pd.read_csv(glob.glob('/project/3013068.03/derivate/fmriprep/{0}/ses-mri03/func/'\
                                                     '{0}_ses-mri03_task-15isomb3TR2020TE28RS*_dir-COL_run-2_echo-1_'\
                                                     'AROMAnoiseICs.csv'.format(participant))[0],header=None)
		...
		nib.save(contrast, '/project/3013068.03/RETROICOR/Example_Visualisation/{0}/Melodic_Matching_corrected/z_map_{0}_{1}.nii.gz'.format(participant, x))
		...
		plotting.plot_glass_brain(contrast, colorbar=True, threshold=norm.isf(0.001), \
                                  ...
                                  '_corrected/z_map_{0}_{1}.png'.format(participant, x))
		plt.close
