...
for subject_long in part_list:
    ...
    mixing_output_file = BASEPATH + \
                         '{0}/3C4R1M_vs_AROMA_corrected/Resting_State_confounds_all_{0}_ses-0{1}_run-02_nonaggrAROMA.txt'.format(sub_id, str(ses_nr))
    ...
    for regressor in sub_physio_columns:
        ...
        np.savetxt(BASEPATH + '{0}/3C4R1M_vs_AROMA_corrected/{1}'.format(sub_id, regressor), single_reg)
    ...
    for mult_counter, multiplications in enumerate(multi_list):
        np.savetxt(BASEPATH + '{0}/3C4R1M_vs_AROMA_corrected/multiplication_term_CPRP_0{1}.txt'.format(sub_id, mult_counter + 1), multiplications)
    ...
    for aroma_confounds in aroma_variables:
        ...
        np.savetxt(BASEPATH + '{0}/3C4R1M_vs_AROMA_corrected/{1}'.format(sub_id , aroma_confounds), confound_selection)