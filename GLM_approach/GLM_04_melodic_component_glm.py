...
for subject in participant_list:
    ...
    melodic_mixing_matrix = pd.read_csv(glob.glob(FMRIPREP_PATH + '{0}/ses-mri0{1}/func/{0}_ses-mri0{1}_task'\
                                                  '-15isomb3TR2020TE28RS*run-2_echo-1_desc-MELODIC_mixing.tsv'.format(sub_id, ses_nr+1))[0],
                                        header = None, sep = '\t')

    #Vector of AROMA classified noise ICs
    sub_noise_components = pd.read_csv(glob.glob(FMRIPREP_PATH + '{0}/ses-mri0{1}/func/'\
                                                '{0}_ses-mri0{1}_task-15isomb3TR2020TE28RS*_dir-COL_run-2_echo-1_'\
                                                'AROMAnoiseICs.csv'.format(sub_id, ses_nr+1))[0],header=None)
    sub_sum = []
    for x in range(0,np.shape(melodic_mixing_matrix)[1]):
        ...
        plotting.plot_glass_brain(contrast, colorbar = True,
                                  ...
                                  plot_abs = False,
                                  display_mode='ortho',
                                  output_file = BASEPATH + '{0}/Melodic_Matching'\
                                  '_corrected/z_map_{0}_{1}.png'.format(sub_id, x))
        plt.close
