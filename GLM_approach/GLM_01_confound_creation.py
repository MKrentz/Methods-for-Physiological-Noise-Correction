...
for subject_long in part_list:
    ...
    output_confound_file = '/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/Resting_State_confounds_all_{0}_ses-0{1}_run-02.txt'.format(sub_id , str(ses_nr))
    ...
    aroma_noise_ics = glob.glob('/project/3013068.03/derivate/fmriprep/{0}/ses-mri0{1}/func/*RS*run-2*AROMAnoiseICs*'.format(sub_id , str(ses_nr)))[0]
    ...
    mixing = pd.read_csv(glob.glob('/project/3013068.03/derivate/fmriprep/{0}/ses-mri0{1}/func/*RS_*run-2*MELODIC_mixing.tsv'.format(sub_id , str(ses_nr)))[0], sep='\t', header=None)
    mixing = mixing.drop(aroma_noise_ics_text, 1)
    mixing_output_file = '/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/Resting_State_confounds_all_{0}_ses-0{1}_run-02_nonaggrAROMA.txt'.format(sub_id , str(ses_nr))
    ...
    output_confound_file = '/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/Resting_State_confounds_all_{0}_ses-0{1}_run-02_combined+nonaggrAROMA.txt'.format(sub_id , str(ses_nr))
    ...
    for regressor in sub_physio_columns:
        ...
        np.savetxt('/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/{1}'.format(sub_id , regressor), single_reg)
    ...
    for mult_counter, multiplications in enumerate(multi_list):
                np.savetxt('/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/multiplication_term_CPRP_0{1}.txt'.format(sub_id , mult_counter+1), multiplications)
    ...
    for aroma_confounds in aroma_variables:
        ...
        np.savetxt('/project/3013068.03/RETROICOR/Example_Visualisation/{0}/3C4R1M_vs_AROMA_corrected/{1}'.format(sub_id , aroma_confounds), confound_selection)
